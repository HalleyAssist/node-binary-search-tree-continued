...
BinarySearchTree.prototype.search = function (key) {
  ...
  const comparison = this.compareKeys(key, this.key)
  if (comparison === 0) { return this.data; }
...
BinarySearchTree.prototype._betweenBounds = function (res, lbm, ubm) {
  if (lbm(this.key) && this.left) this.left._betweenBounds(res, lbm, ubm)
  if (lbm(this.key) && ubm(this.key)) { 
    for(const r of this.data){
      res.push(r)
    }
  }
  if (ubm(this.key) && this.right) this.right._betweenBounds(res, lbm, ubm)
}
