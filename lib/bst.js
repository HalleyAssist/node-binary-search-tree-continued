...
class BinarySearchTree extends Array {
  /**
   * Constructor
  ...
  insert(key, value) {
    ...
    if (!this.hasOwnProperty('key')) {
      this.key = key;
      customUtils.isDef(value) && this.push(value);
      return;
    }
    ...
    if (comparison === 0) {
      if (this.unique) {
        ...
        if (this.vkUnique && this.includes(value)) return
        this.push(value);
      }
      return;
    }
  ...
  **/
  ...
  _betweenBounds(res, lbm, ubm) {
    let lRes = lbm(this.key)
